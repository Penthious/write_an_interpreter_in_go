...
func TestModify(t *testing.T) {
	...
	for _, tt := range tests {
		...
		equal := reflect.DeepEqual(modified, tt.want)

		if !equal {
			t.Errorf("not equal, got=%#v, want=%#v", modified, tt.want)
		}
	}
	...
	for key, val := range hashLiteral.Pairs {
		...
		if key.Value != 2 {
			t.Errorf("key.Value in not %d, got=%d", 2, key.Value)
		}
		val, _ := val.(*IntegerLiteral)
		if val.Value != 2 {
			t.Errorf("val.Value in not %d, got=%d", 2, val.Value)
		}
	}
}
